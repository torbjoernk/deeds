...
$ ->
  DeedsApp.display_show_modal(
    ...
    "<%= escape_javascript(render partial: 'mention/mention_entries/show/content', locals: { native: true }) %>",
    "<%= escape_javascript(render partial: 'shared/show/footer',
                                  locals: { edit_link_path: edit_mention_mention_entry_path(@mention_entry) }) %>"
  )
